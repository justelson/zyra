...
assert.match(flowSource, /onboarding-step-scroll[\s\S]*onboarding-step-content[\s\S]*onboarding-step-heading/, 'step titles and choices share one centered scrolling unit')
...
assert.match(flowSource, /stepScrollRef\.current\.scrollTop = 0/, 'each centered step must open at the top of its own scroll region')
...
assert.match(signInSource, /<ZyraLogoASCII/, 'sign-in uses the existing Zyra mark')
...
const welcomeStepSource = stepsSource.split('export function WelcomeStep')[1]?.split('export function AppearanceStep')[0] || ''
