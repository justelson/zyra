...
import { BrowserSurfaceInbox } from './browser-surface-inbox'
...
export function createAgentControlAdapter() {
    const surfaceInbox = new BrowserSurfaceInbox<BrowserSurfaceOpenRequest>()
    ipcRenderer.on(AGENT_CONTROL_IPC.browserSurfaceRequested, (_event, request) => surfaceInbox.receive(request))
    ipcRenderer.on(AGENT_CONTROL_IPC.browserSurfaceCancelled, (_event, requestId) => surfaceInbox.cancel(requestId))
    return {
        ...
        onBrowserSurfaceRequest: (callback: (request: BrowserSurfaceOpenRequest) => void) => surfaceInbox.subscribe(callback),
