...
| FIX-012 | Local and live fixes verified; independent workflow QA passed | Windows computer-use pointer feedback follows native progress, supports negative display coordinates and converts physical pixels to display coordinates. The full-screen glow extends deeper with soft falloff and smooth entrance/exit. | Cursor, native ownership/cancellation, sequence contracts and Desktop regressions pass. Per-monitor awareness aligns startup bounds, UIA and capture at 125% scaling. Explicit focus preserves maximized bounds; agent Escape preserves the grant. Elapsed-time drag pacing and bounded read-only pointer confirmation avoid accumulated delays and false placement failures without replay. Routine coordinate clicks now batch with semantic actions under the same checks. Final independent fresh-context background-window drawing scored 9/10: 127 seconds, five computer calls, zero errors, with real progress verified after every batch. Earlier cancelled-shape work prompted corrected edit-completion guidance. Separate actual-overlay fixtures measured first-show delay falling from 220 to 15 ms and synthetic-progress-to-animation-frame median latency from 33 to 4 ms. The rating covers the tested workflow; full compositor cadence and non-primary-monitor live behavior remain unverified. |
