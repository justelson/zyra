...
| FIX-012 | Local and live fixes verified; experience target pending | Windows computer-use pointer feedback follows native progress, supports negative display coordinates and converts physical pixels to display coordinates. The full-screen glow extends deeper with soft falloff and smooth entrance/exit. | Cursor, sidecar, native ownership/cancellation and Desktop regressions pass. Per-monitor awareness aligns startup bounds, UIA and capture at 125% scaling. Explicit focus preserves maximized bounds; agent Escape preserves the grant. Elapsed-time drag pacing avoids accumulated delays, and bounded read-only pointer confirmation handles delayed Windows feedback without replaying input. Latest independent fresh-context background-window drawing completed in 133 seconds with four computer calls and zero errors (about 62% faster than the original 349-second run), rated 8.5/10. The requested 9/10 experience target remains unmet; provider waiting and visible cursor cadence remain limitations. The capture used for live cursor comparison ran below its requested frame rate. A separate actual-overlay fixture measured first-show delay falling from 220 to 15 ms by preparing the hidden cursor at grant activation. Publishing already-eligible updates immediately reduced synthetic-progress-to-animation-frame median latency from 33 to 4 ms while preserving coalescing and cancellation. Full compositor cadence remains unverified. |
