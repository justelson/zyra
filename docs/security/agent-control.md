...
| Chrome ambient authority | MV3 declares `debugger`, `tabs`, `storage`, `webNavigation`, and `favicon`, with no broad host permissions. Pairing requests optional `http://127.0.0.1/*` access. Debugger attachment requires a separate explicit Read or Control tab selection. Read access cannot produce input; grants remain exact-tab and origin-bound. Same-origin navigation invalidates references; a changed origin revokes sharing. Worker restart and disconnect clear input grants. |
...
A bounded redacted observation is returned in the calling Chat's tool transcript because it is the agent's feedback for the next action. Model-facing Windows enumeration requires a non-empty application query; unfiltered legacy requests are rejected. Pre-grant search results omit ambient window titles, and raw candidate arrays are not copied into persisted tool details. The common `computer_use_app` path may combine registered-app reuse or launch, exact single-match selection, grant request, and observation in one tool call. Optional routine steps still pass the same sequence validator, capability checks, revisions, action counts, and per-step audits before the call returns its final observation. Before embedded typing in a newly launched app, the broker also requires the exact writable target to be provably blank and the window not to present as dirty; restored, non-empty, or unreadable content blocks before input and revokes the new grant. A missing role hint is accepted only when one non-sensitive control with the exact case-sensitive name supports the requested action; zero or multiple matches fail closed. For a multi-app root turn, the broker revokes an older Windows grant only after the next app's exact grant succeeds. Multiple matches still require explicit candidate selection. A Windows grant returns its initial observation and bounded selected-window identity. When screenshot access is granted, acquisition includes the current selected-window image; embedded batches capture only the final revision. Routine action sequences revalidate semantic role/name matches and commit a fresh observation after every semantic click, target-relative coordinate click, exact-field type, safe editing/navigation key, bounded target-relative drag, or short wait. A click_point step performs one left click and can share a batch with subsequent tool selection. A sequence has at most 16 steps; its bridge deadline accounts for each bounded action and fresh observation, and expiry sends correlated cancellation without replay. Coordinate clicks and drags require a fresh viewport, reject points overlapping observed sensitive or critical controls, and use the same native point and window checks as individual input. Each click still requires pointer.click in the exact current grant. They cannot carry critical side effects, target sensitive elements, use system shortcuts, or bypass grant expiry, action limits, capabilities, revision checks, audit, interruption, or Emergency Stop.
...
Windows observations rebuild bounded UI Automation caches on each read and expose compact control rows with explicit column names. Each row retains its current element reference, role, name, actions and optional value/state details; focus, truncation and redaction metadata remain attached. Supported selection, toggle and expansion states are included from the same fresh cache; unavailable patterns remain unknown. This lets agents distinguish an active tool or checked control without toggling it again. Values are read only after sensitivity classification and a fresh password check. No element reference is reused across observation revisions. If a profile picker closes, the agent rediscovers the requested application and requests a fresh exact-window grant; no failed action is replayed or grant transferred.

Target-relative pointer move, coordinate click, and drag are available only with their explicit capabilities and the latest observation revision. The native helper declares per-monitor DPI awareness from startup so window bounds, UIA and capture share physical pixels before and after the first observation. Electron translates coordinates to screen space; the sidecar then verifies each point is inside the selected window and that `WindowFromPoint` still resolves to that exact top-level target. When the current grant explicitly includes window focus, native coordinate input first activates only that selected window. Activation preserves a maximized window; the observed physical rectangle must still match before and after focus or input stops for a fresh observation. Without focus permission, Zyra keeps the existing pointer ownership checks and does not bring a covered window forward on the agent's behalf. Obscured, replaced, out-of-window, higher-integrity, or mispositioned pointer input fails before mouse-down. Drag side effects use the same canonical approval rules as clicks, typing, keys, and selection.

While an exact Windows grant is active, Electron presents two click-through overlays derived only from that granted target. A capture-visible synthetic Zyra cursor follows request-correlated native pointer progress for physical input and already-observed UI Automation bounds for semantic actions. Its document loads hidden when an exact Windows grant becomes active, so the first update does not wait for renderer startup; preparation alone never displays a cursor. Eligible cursor updates publish immediately once the frame interval has elapsed; only updates arriving within that interval wait and coalesce, avoiding an extra Windows timer tick. Physical screen positions, including negative monitor coordinates, are converted to display coordinates; the cursor surface follows its current display. Failed gestures retain their last reported position rather than predicting the requested endpoint. Drag pacing follows elapsed deadlines and skips overdue frames, so native work and timer rounding do not accumulate into a longer gesture; each emitted point still passes the same ownership and cancellation checks. After a single positioning command, native confirmation can wait up to 100 milliseconds for Windows to report the requested point only while the cursor remains at its previous position. This read-only wait never replays input; unexpected movement, lost ownership, or cancellation stops it immediately. A separate capture-protected, full-display edge glow appears on the display containing the selected app and identifies the application in a spacious status treatment beside the stop key. Its transparent center does not tint the controlled app, while its status surface, text, font, and accent follow Zyra's current appearance preferences. The exact grant remains window-bound even though the safety treatment fills the display. Neither overlay accepts input, grants authority, changes revisions, appears in selected-window screenshots, or exposes another window. Cursor feedback and the status label stop immediately on release, expiry, target loss, turn interruption, sidecar failure, shutdown, or Emergency Stop. The decorative edge glow eases out independently; its animation never delays revocation or native input cancellation. A broker-owned expiry sweep revokes idle authority and releases retained targets even when no UI read or action occurs; expired pending approvals are settled the same way.

## Chrome setup and development

The installed app includes the extension. Open **Settings → Device connections → Chrome browser → Extension folder**, load that folder from Chrome's `chrome://extensions` page, then connect using Zyra's eight-digit code and port. Users select individual tabs and Read or Control access in the popup. No standalone bridge or MCP installation is required.

Developers first run `npm ci --prefix extensions/zyra-browser-control` and `npm --prefix extensions/zyra-browser-control run package`. The build produces an unpacked folder and deterministic ZIP. Chrome Web Store publication and automatic installation remain separate release work.

Chrome actions use `browser_act`; staged `browser_perform` remains specific to the in-app Browser. Chrome dragging can focus the selected tab only with a `window.focus` grant, avoiding background-tab input throttling. Without that capability, an inactive-tab stroke fails before mouse-down. Correlated cancellation removes queued requests or aborts delivered requests, and late responses cannot poison token rotation or replay input. Only appearance fields cross the pairing transport for the extension's Follow Zyra theme.
...
- Keyboard: plain `Escape` while an exact Windows grant is active, plus the permanent `Ctrl+Alt+Escape` fallback (`Command+Alt+Escape` on macOS, though Windows control is Windows-only). Plain Escape is unregistered immediately when Windows control ends. Agent-requested unmodified Escape is delivered only to the validated focused control of the exact selected window, so dismissing an app popup does not trigger the physical-key emergency stop. Other keyboard input retains its native synthesis path.
